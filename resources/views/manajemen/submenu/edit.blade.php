                    ...
                    <div class="card card-flush h-lg-100" id="kt_contacts_main">
                        <div class="card-header pt-7" id="kt_chat_contacts_header">
                            <div class="card-title">
                                ...
                                <h2>{{ $title }} {{ $stmtSubMenu->nama }}</h2>
                            </div>
                        </div>
                        <div class="card-body pt-5">
                            <form action="{{ route('manajemen-submenu.update', $stmtSubMenu->id) }}" class="form"
                                method="POST">
                                @method('put')
                                @csrf
                                <div class="fv-row mb-7">
                                    <label for="id_menu" class="fs-6 fw-semibold form-label mt-3">
                                        <span class="required">Menu</span>
                                    </label>
                                    <select class="form-select @error('jenis_dokumen_id') is-invalid @enderror"
                                        id="id_menu" name="id_menu" data-control="select2"
                                        data-placeholder="---Pilih Menu---">
                                        <option></option>
                                        @foreach ($stmtMenu as $menu)
                                            <option value="{{ $menu->id }}"
                                                {{ old('id_menu', $stmtSubMenu->id_menu) == $menu->id ? 'selected' : '' }}>
                                                {{ $menu->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="fv-row mb-7">
                                    <label for="nama" class="fs-6 fw-semibold form-label mt-3">
                                        <span class="required">Nama SubMenu</span>
                                        <i class="fas fa-exclamation-circle ms-1 fs-7" data-bs-toggle="tooltip"
                                            title="Nama SubMenu tidak boleh sama dengan yang sudah ada dan maksimal 100 karakter"></i>
                                    </label>
                                    <input type="text"
                                        class="form-control form-control-solid @error('nama') is-invalid @enderror"
                                        name="nama" value="{{ old('nama', $stmtSubMenu->nama) }}" id="nama" />
                                    @error('nama')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="fv-row mb-7">
                                    <label for="link" class="fs-6 fw-semibold form-label mt-3">
                                        ...
                                        <i class="fas fa-exclamation-circle ms-1 fs-7" data-bs-toggle="tooltip"
                                            title="Contoh : /example"></i>
                                    </label>
                                    <input type="text"
                                        class="form-control form-control-solid @error('link') is-invalid @enderror"
                                        id="link" name="link" value="{{ old('link', $stmtSubMenu->link) }}" />
                                    @error('link')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                ...
                                <div class="fv-row mb-7">
                                    <label for="urutan" class="fs-6 fw-semibold form-label mt-3">
                                        <span class="required">Urutan SubMenu</span>
                                        <i class="fas fa-exclamation-circle ms-1 fs-7" data-bs-toggle="tooltip"
                                    ...
                                    <input type="number" min="1"
                                        ...
                                        id="urutan" name="urutan" value="{{ old('urutan', $stmtSubMenu->urutan) }}" />
                                    @error('urutan')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
